 ...
 *
 * Handles user authentication for the Radius app.
 * Provides a tabbed interface to switch between Sign In and Sign Up modes.
 * Sign-up is a two-step flow: Step 1 collects email & password,
 * Step 2 collects the user’s first and last name.
 * Includes animated tab indicator, crossfade transitions, and input validation.
 */
...
export default function Login() {
	...
	/** Current step in the sign-up flow (1 = credentials, 2 = name) */
	const [signUpStep, setSignUpStep] = useState(1);
	/** Client-side validation error displayed to the user */
	...
	/** RFC 5322-ish email regex for basic client-side validation */
	const emailPattern =
	...
	// Animated opacity for the login button
		...
		>
			<View className="w-full bg-[#FFF8F0]">
				...
				<View className="mx-8 mt-8 flex flex-col bg-[#FFF8F0]">
					...
					{/* Submit / Next button */}
					<Pressable
						className="mt-2 rounded-xl mb-6"
						onPress={() => {
							// Animate button press feedback (opacity dip)
							...
							if (login) {
								// Sign-In mode: attempt authentication immediately
								signIn(email, password);
							} else if (signUpStep === 1) {
								// Sign-Up step 1: validate email/password before advancing
								if (!emailPattern.test(email.trim())) {
									...
									setSignUpStep(2); // Advance to name collection
								}
							} else {
								// Sign-Up step 2: validate name, then register
